<template>
    ...
    <div class="inspection-list-page">
        <div class="page-head">
            ...
            <div>
               <tw-search type="lease" :searchList="searchList" ></tw-search>
            </div>
        ...
        <div class="page-paging">
            <tw-pagination type="lease" @upDatePageIndex="upDatePageIndex"></tw-pagination>
        </div>
        <div class="page-button">
            <tw-button title="修改" @click="changeLease"></tw-button>
            <tw-button title="匯出" @click="remit"></tw-button>
        </div>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        getInspectByDate(start,end){
            this.$axios.get(API.getLease+start+'_'+end)
            .then(res => {
                const data = res.data;
                this.disposeData(data);
            })
            .catch(err => {
                console.error("場地清單接口錯誤")
            })
        },
        disposeData(data){
            this.$store.commit('addData',{key: 'leaseAllData',value: data})
            this.$store.commit('addData',{key: 'leaseFilterData',value: data})
            this.$store.commit('addData',{key: 'leaseFindData',value: data.slice(0,12)})
        },
        handleSearch(obj){
        ...
        selectType(obj){
            ...
            switch (obj.type) {
                case 0:
                    data = this.$store.state.leaseAllData;
                    this.$store.commit('addData',{key: 'leaseFilterData',value: data});
                    break;
                case 1:
        ...
        filterData(_type) {
            let data = this.$store.state.leaseAllData;
            let arr= []
            data.forEach( item => {
        ...
        changeLease(){
            ...
            console.log( this.$store.state.storageSelectData[0].ID,'this.$store.state.storageSelectData[0].idthis.$store.state.storageSelectData[0].id')
        ...
        sortByBeginTime(){
            if(this.beginTimeSort){
                this.$store.commit('reverseList','lease')
                this.beginTimeSort = false;
            }else{
                this.$store.commit('sortByBeginTime','lease')
        ...
        sortByEndTime(){
            if(this.endTimeSort){
                ...
                this.$store.commit('sortByEndTime','lease')
        ...
        sortBySite(){
            if(this.siteSort){
                ...
                this.$store.commit('sortBySite','lease')
                this.siteSort = true;
            }
            this.beginTimeSort = false;
            this.endTimeSort = false;
    ...
    computed: {
        dataList(){
            return this.$store.state.leaseFindData
        }
    },
    filters: {
        dateToDate(date){
